...
5. **Key bindings** — a short prose explanation first: Shift gives uppercase keys as usual; Space is a held modifier — hold Space, press another key to fire a chord action. Then the keyboard layout diagram and legend copied verbatim from `keybindings.md`. No redesign — the existing diagram already matches the in-app overlay and is maintained there.
